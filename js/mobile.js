...
(function(){
  ...
  if(mMenu && burger){
    ...
    mMenu.addEventListener('click', function(e){
      if(e.target.closest('a,button')){ document.body.style.overflow = ''; if(burger) burger.setAttribute('aria-expanded', 'false'); }
    });
    if(isTouch){
      var mx0 = null;
      ...
      mMenu.addEventListener('touchend', function(e){
        ...
        if(dx > 70){ mMenu.classList.remove('open'); document.body.style.overflow = ''; if(burger) burger.setAttribute('aria-expanded', 'false'); }
        mx0 = null;
      }, {passive:true});
    }
  }
